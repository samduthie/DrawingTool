...
public class TaxiCo
{
    ...
    /**
     * Returns an available shuttle or taxi to take passanger to destination. 
     * Free shuttles are selected first to save passangers money
     */
    public Vehicle bookVehicle(String destination)
    {
        //first we check through the list of shuttles
        for (Vehicle checkVehicles: vehicleFleet){
             if (checkVehicles instanceof Shuttle){
                 if (checkVehicles.getDestination() == destination){
                     return checkVehicles;
                }
            }
        }
        //if no shuttles are found we check our taxis to see if any are free
        for (Vehicle checkVehicles: vehicleFleet){
                if (checkVehicles instanceof Taxi && checkVehicles.getDestination() == null){
                    return checkVehicles;
                }
            }
        return null;
